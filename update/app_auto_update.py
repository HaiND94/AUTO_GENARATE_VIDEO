...
class Git:
    ...
    def git_pull(self):
        ...
        subprocess.run("pip install yt-dlp --upgrade", shell=True)
...
def auto_update(api_cli):
    ...
    try:
        notice_versionApi = VersionApi(api_client=api_cli)
        version_Api = VersionSeverApi(api_client=api_cli)
    except Exception as e:
        logging.info(e)
        return False
    ...
    time.sleep(5)
    if not os.path.isdir("./outside_lib"):
        pass
    else:
...
if __name__ == "__main__":
    schedule.every(2).hours.do(auto_update)
    while True:
